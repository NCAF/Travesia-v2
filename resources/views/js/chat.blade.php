...
<script>
    $(document).ready(function() {
        ...
        function get_list_chat(search = '') {
            ...
            callApi("GET", "{{ route('api.chat.user') }}"+search, param, function (req) {
                list = '';
                    ...
                    <button class="list-group-item list-group-item-action border-0 mb-3 p-2 btn-chat-list" data-nama="${val.nama_channel}" data-id="${val.id}" data-foto="${val.foto}">
                        <div class="d-flex align-items-start">
                            ...
                            <div class="flex-grow-1 ms-3">
                                ${val.nama_channel}
                                <div class="small ${val.status=='opened'?'chat-online':'chat-offline'}"><i class="fa-solid fa-circle"></i> ${val.status}</div>
                            </div>
                        </div>
                    </button>
                    `;
